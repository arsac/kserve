...
Adds the SupportsHMA mixin to LMCacheConnectorV1Dynamic so that vLLM's
hybrid KV cache allocator works with LMCache. This enables sliding-window
layers to allocate only their window size instead of the full context.
...
mod = importlib.import_module("lmcache.integration.vllm.lmcache_connector_v1")
path = mod.__file__
